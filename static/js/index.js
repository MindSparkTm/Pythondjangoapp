...
function populatePrevVisit(uuid, triage_id) {
    ...
    $.ajax({
        ...
        success: function (visit) {
            console.log(visit)
            if (visit.length != 0) {
                $("#prev_diagnosis").text(visit.diag_search)
                $("#prev_summary").text(visit.plan_of_managemnt)
                $("#prev_date").text(moment(visit.last_updated))
                $("#prev_his_illness").text(visit.his_presenting_illness)
                $("#prev_examination").text(visit.current_diagnosis)
                $("#prev_query_diag").text(visit.query_diagnosis)
                $("#prev_diag").text(visit.diag_search)
...
$(function () {
    $('#livesearch').on('click', 'td', function () {
        ...
        var dialog = bootbox.dialog({
            ...
            message: "<input onkeyup='tablets=$(this).val()' class='form-control' id='tablets' placeholder='Dose'><br><input  onkeyup='freq=$(this).val()' class='form-control' id='freq' placeholder='Interval'><br><input  onkeyup='route=$(this).val()' class='form-control' id='route' placeholder='Route'><br><input onkeyup='days=$(this).val()' class='form-control' id='days' placeholder='Duration'>",
            buttons: {
                cancel: {
                    label: "cancel",
                ...
                ok: {
                    ...
                    callback: function () {
                        ...
                        var dosage = '|'+ tablets + '| ' +  + freq + '| ' +  days +'|'+route+";;"
